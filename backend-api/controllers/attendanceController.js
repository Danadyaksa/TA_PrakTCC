...
exports.checkOut = async (req, res) => {
  const { attendance_id, latitude, longitude, selfie_url, liveness_score } = req.body;
  const user_id = req.user.id;
  
  try {
    // 1. Update attendance with check-out time
    ...
    // 2. Save Check-out Logs to Firestore
    if (selfie_url) {
      await firestore.collection('selfie_logs').add({
        attendance_id,
        user_id,
...
exports.getDailySummary = async (req, res) => {
  ...
  try {
    ...
    // 3. For each row without an attendance record, mark as alpha
...
exports.getMonthlySummary = async (req, res) => {
  ...
  try {
    ...
    for (const user of usersResult.rows) {
      ...
      const days = [];
      let countHadir = 0, countTerlambat = 0, countAlpha = 0, countIzin = 0;
      const totalWorkDays = Object.keys(schedulesByDay).length > 0 ? lastDay : 0;
      ...
      for (let d = 1; d <= lastDay; d++) {
        ...
        if (onLeave) {
          ...
          // No attendance and no leave = alpha (only if date has passed)
